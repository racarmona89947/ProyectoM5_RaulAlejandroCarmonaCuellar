...
export function CheckoutPage() {
  ...
  async function handleConfirm() {
    ...
    try {
      const orderId = await createOrder(firebaseUser.uid, items);
      ...
      const message =
        ...
        checkoutError.message.startsWith("INSUFFICIENT_STOCK")
          ? "Algunos productos ya no tienen suficiente stock. Ajusta tu carrito e intenta nuevamente."
          : "No pudimos crear la orden. Tu carrito sigue intacto; intenta nuevamente.";
      setError(message);
  ...
  return (
    <main className="min-h-screen bg-[var(--page)]">
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
        <Link className="font-semibold text-[var(--royal-violet)]" to="/cart">
          ← Volver al carrito
        </Link>
        <section className="mt-8 rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-6 sm:p-8">
          ...
          <h1 className="mt-2 font-[Space_Grotesk] text-3xl font-bold text-[var(--text)]">
            Confirma tu compra
          </h1>
          ...
          <div className="mt-6 flex items-center justify-between text-xl font-bold text-[var(--text)]">
            <span>Total</span>
          ...
          {error && (
            <p
              className="mt-5 rounded-lg bg-red-50 p-3 text-sm text-[var(--danger)]"
              role="alert"
            >
              {error}
            </p>
          ...
          <button
            className="mt-8 w-full rounded-xl bg-[var(--royal-violet)] px-4 py-3 font-semibold text-white disabled:opacity-60"
