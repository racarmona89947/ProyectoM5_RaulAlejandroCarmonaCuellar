...
import { createProduct, deleteProduct, getAllOrders, getAllProducts, updateOrderStatus, updateProduct, type ProductInput } from '../services/adminService'
...
export function AdminPage() {
  ...
  return <main className="min-h-screen bg-[var(--page)] px-4 py-8 text-[var(--text)] sm:px-6"><div className="mx-auto max-w-7xl">
    ...
    {isOrdersView ? <OrdersSection filter={filter} isLoading={isLoading} orders={visibleOrders} onFilterChange={setFilter} onStatusChange={(orderId, status) => void handleOrderStatus(orderId, status)} /> : <ProductsSection categories={categories} editingId={editingId} form={form} isLoading={isLoading} isSaving={isSaving} products={products} onCancel={() => { setEditingId(null); setForm(emptyProduct) }} onDelete={(productId) => void handleDelete(productId)} onEdit={(product) => { setEditingId(product.id); setForm({ name: product.name, brand: product.brand, description: product.description, category: product.category, price: product.price, stock: product.stock, images: product.images, featured: product.featured }) }} onFormChange={setForm} onSubmit={handleSubmit} />}
  </div></main>
}

interface ProductsSectionProps { categories: string[]; editingId: string | null; form: ProductInput; isLoading: boolean; isSaving: boolean; products: Product[]; onCancel: () => void; onDelete: (productId: string) => void; onEdit: (product: Product) => void; onFormChange: (form: ProductInput) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }

function ProductsSection({ categories, editingId, form, isLoading, isSaving, products, onCancel, onDelete, onEdit, onFormChange, onSubmit }: ProductsSectionProps) {
  return <div className="mt-8 grid gap-8 xl:grid-cols-[380px_1fr]">
    <section className="h-fit rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm"><h2 className="font-[Space_Grotesk] text-xl font-bold">{editingId ? 'Editar producto' : 'Nuevo producto'}</h2><form className="mt-5 space-y-4" onSubmit={onSubmit}><input className="w-full rounded-lg border border-[var(--border)] bg-transparent px-3 py-2" onChange={(event) => onFormChange({ ...form, name: event.target.value })} placeholder="Nombre" required value={form.name} /><input className="w-full rounded-lg border border-[var(--border)] bg-transparent px-3 py-2" onChange={(event) => onFormChange({ ...form, brand: event.target.value })} placeholder="Marca" required value={form.brand} /><select className="w-full rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 py-2" onChange={(event) => onFormChange({ ...form, category: event.target.value })} required value={form.category}><option value="" disabled>Selecciona una categoría</option>{categories.map((category) => <option key={category} value={category}>{category}</option>)}</select><textarea className="w-full rounded-lg border border-[var(--border)] bg-transparent px-3 py-2" onChange={(event) => onFormChange({ ...form, description: event.target.value })} placeholder="Descripción" required value={form.description} /><div className="grid grid-cols-2 gap-3"><input className="w-full rounded-lg border border-[var(--border)] bg-transparent px-3 py-2" min="0" onChange={(event) => onFormChange({ ...form, price: Number(event.target.value) })} placeholder="Precio" required type="number" value={form.price || ''} /><input className="w-full rounded-lg border border-[var(--border)] bg-transparent px-3 py-2" min="0" onChange={(event) => onFormChange({ ...form, stock: Number(event.target.value) })} placeholder="Stock" required type="number" value={form.stock || ''} /></div><input className="w-full rounded-lg border border-[var(--border)] bg-transparent px-3 py-2" onChange={(event) => onFormChange({ ...form, images: event.target.value ? [event.target.value] : [] })} placeholder="URL de imagen" type="url" value={form.images[0] ?? ''} /><label className="flex items-center gap-2 text-sm"><input checked={form.featured} onChange={(event) => onFormChange({ ...form, featured: event.target.checked })} type="checkbox" /> Producto destacado</label><button className="w-full rounded-lg bg-[var(--royal-violet)] px-4 py-3 font-semibold text-white disabled:opacity-60" disabled={isSaving} type="submit">{isSaving ? 'Guardando...' : editingId ? 'Guardar cambios' : 'Crear producto'}</button>{editingId && <button className="w-full rounded-lg border border-[var(--border)] px-4 py-3 font-semibold" onClick={onCancel} type="button">Cancelar edición</button>}</form></section>
