...
export function AdminPage() {
  ...
  async function loadAdminData() {
    ...
    try {
      ...
      setError(
        "No pudimos cargar los datos del panel. Verifica las reglas de Firestore.",
      );
    } finally {
      setIsLoading(false);
    }
    ...
    try {
      ...
      setError(
        saveError instanceof Error &&
          saveError.message === "Could not upload image"
          ? "No pudimos subir la imagen a S3."
  ...
  return (
    <main className="min-h-screen bg-[var(--page)] px-4 py-8 text-[var(--text)] sm:px-6">
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-6 text-sm font-semibold uppercase tracking-[0.16em] text-[var(--indigo-bloom)]">
              Administración
            </p>
            <h1 className="mt-1 font-[Space_Grotesk] text-3xl font-bold">
              {isOrdersView ? "Órdenes" : "Catálogo de productos"}
            </h1>
            <p className="mt-2 text-[var(--text-muted)]">
              {isOrdersView
                ? "Revisa y actualiza el estado de las compras."
                : "Crea, edita y organiza los productos de tu marketplace."}
            </p>
          </div>
          <button
            className="rounded-xl border border-[var(--border)] bg-[var(--surface)] px-4 py-2 text-sm font-semibold transition hover:border-[var(--royal-violet)]"
        ...
        {error && (
          <p
            className="mt-6 rounded-lg bg-red-50 p-3 text-[var(--danger)]"
            role="alert"
          >
            {error}
          </p>
        ...
        {isOrdersView ? (
          <OrdersSection
            ...
            onStatusChange={(orderId, status) =>
              void handleOrderStatus(orderId, status)
            }
          />
        ) : (
          <ProductsSection
...
interface ProductsSectionProps {
  ...
  onFormChange: (form: ProductInput) => void;
...
function ProductsSection({
  ...
  return (
    <div className="mt-8 grid gap-8 xl:grid-cols-[380px_1fr]">
      <section className="h-fit rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm">
        ...
        <form className="mt-5 space-y-4" onSubmit={onSubmit}>
          ...
          <div className="grid grid-cols-2 gap-3">
            <input
              ...
              min="0"
              onChange={(event) =>
                onFormChange({ ...form, price: Number(event.target.value) })
              }
              placeholder="Precio"
              required
              type="number"
            ...
            <input
              ...
              min="0"
              onChange={(event) =>
                onFormChange({ ...form, stock: Number(event.target.value) })
              }
              placeholder="Stock"
              required
              type="number"
          ...
          <label className="flex items-center gap-2 text-sm">
            <input
              ...
              onChange={(event) =>
                onFormChange({ ...form, featured: event.target.checked })
              }
              type="checkbox"
            />{" "}
            Producto destacado
          ...
          <button
            className="w-full rounded-lg bg-[var(--royal-violet)] px-4 py-3 font-semibold text-white disabled:opacity-60"
            disabled={isSaving}
            type="submit"
          >
            {isSaving
      ...
      <section>
        ...
        {isLoading ? (
          <p className="mt-4 text-[var(--text-muted)]">Cargando productos...</p>
        ) : (
            ...
            {products.map((product) => (
              ...
              >
                ...
                <div className="mt-4 flex gap-3">
                  <button
                    className="text-sm font-semibold text-[var(--royal-violet)]"
...
function OrdersSection({
  ...
  return (
    <section className="mt-8">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-sm text-[var(--text-muted)]">
            Gestiona el flujo de compras desde un solo lugar.
          </p>
        ...
        <select
          ...
          onChange={(event) =>
            onFilterChange(event.target.value as "all" | OrderStatus)
          }
          value={filter}
        >
      ...
      {isLoading ? (
        <p className="mt-6 text-[var(--text-muted)]">Cargando órdenes...</p>
      ) : (
        <div className="mt-5 space-y-3">
          ...
          {orders.length === 0 && (
            <p className="rounded-2xl border border-dashed border-[var(--border)] p-8 text-center text-[var(--text-muted)]">
