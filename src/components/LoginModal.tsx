...
export function LoginModal({ onClose, onOpenRegister }: LoginModalProps) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setError("No pudimos iniciar sesion. Revisa tus datos.");
      showToast("No pudimos iniciar sesión. Revisa tus datos.", "error");
    } finally {
  ...
  async function googleLogin() {
    ...
    try {
      ...
      showToast("No pudimos iniciar sesión con Google.", "error");
    } finally {
      setIsSubmitting(false);
    }
  }
  ...
  return (
    <div
      aria-label="Ventana de inicio de sesion"
      aria-modal="true"
    ...
    >
      <div className="relative w-full max-w-md rounded-3xl border border-[var(--border)] bg-[var(--surface)] p-6 shadow-2xl">
        ...
        <button
          className="mt-3 w-full rounded-xl border border-[var(--border)] px-4 py-3 font-semibold text-[var(--text)]"
          disabled={isSubmitting}
